...
csvfiles = open('./data/movies.csv', newline='', encoding="utf-8")
...
for i in range(1, len(ratings_matrix)):
    ...
    for j in range(len(relation_matrix)):
        if (movie_diction[ratings_matrix[i][1]] == j):
            ...
            movies_matrix[j+1][3][0] += (float(ratings_matrix[i][2]) * scale)
            movies_matrix[j+1][3][1] += scale
        else:
            #'''
            scale = round(rating_time/current_timestamp, 1) * relation_matrix[movie_diction[ratings_matrix[i][1]]-1][j] / len(movies_matrix) * 5
            movies_matrix[j+1][3][0] += (float(ratings_matrix[i][2]) * scale)
            movies_matrix[j+1][3][1] += scale
            #'''
    print(i)
...
print(current_timestamp)
'''
with open('./outputRating.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
...
'''
for i in range(1, len(movies_matrix)):
    if(movies_matrix[i][3][1] <= 0.9 and movies_matrix[i][3][1] != 0):
        print(movies_matrix[i][0], movies_matrix[i][3][0], movies_matrix[i][3][1], movies_matrix[i][3][0]/movies_matrix[i][3][1])
'''
